...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{6cm}p{9cm}@{}}
  ...
  \midrule
  \multicolumn{2}{l}{\textit{非三角面跳过 + 警告}} \\
  \texttt{sqrt3\_skips\_non\_triangle\_with\_warning} & 单四边形面输入：跳过，输出 0 面（stderr 警告） \\
  \bottomrule
\end{longtable}
}

共 16 个单元测试，全部通过。全库 \texttt{cargo test} 共 498 个用例，0 失败。
