...
用 Cramer 法则求解。奇异性判定使用\textbf{相对阈值}：
\[
  |\det A| < \varepsilon \cdot \|A\|_F^3, \quad \varepsilon = 10^{-12}
\]
其中 $\|A\|_F$ 是 3$\times$3 块的 Frobenius 范数。相对阈值在不同坐标量级
（小网格 $\det \sim 1$，大网格 $\det \sim 10^{12}$）下均能正确区分奇异与非奇异，
而绝对阈值 $10^{-12}$ 在大网格上会把可解系统误判为奇异。$\|A\|_F = 0$
（零矩阵）直接判奇异。
...
\begin{itemize}
  ...
  \item 差值 $> 10^{-9} \cdot \max(|heap\_cost|, |stored\_cost|, 1)$ 则为过期条目，跳过
        （\textbf{相对容差}：绝对阈值在大坐标网格上 cost $\sim 10^6$ 时
        会把合法条目误判为过期）；
...
\begin{itemize}
  \item \textbf{除零}：Cramer 法则检查 $|\det| < 10^{-12} \cdot \|A\|_F^3$（相对阈值）返回 \texttt{None}；
