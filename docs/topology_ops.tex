...
\begin{tabular}{@{}ll@{}}
  ...
  \texttt{FlipCreatesNonManifoldEdge\{c, d\}} & 翻转后对角线两端已相连（产生重复边）\\
  \texttt{LinkConditionViolated\{a, b\}} & 公共邻居 $\neq \{C, D\}$ \\
  \texttt{CollapseOnBoundaryVertices\{a, b\}} & 折叠边两端点均为边界顶点 \\
...
\texttt{TopologyError::Inconsistent(msg)} 中的 \texttt{msg} 字符串\textbf{统一使用中文}，
与代码库其余部分（\texttt{io.rs} 各 \texttt{*Error}、\texttt{validate.rs}、
\texttt{panic!} / \texttt{eprintln!} / \texttt{expect()}）保持一致。例如：
\begin{itemize}
  \item 字段缺失类：\texttt{"h.next 为 None"}、\texttt{"twin.prev 为 None"}、
        \texttt{"p1.twin 为 None"}；
  \item 实体缺失类：\texttt{"顶点 A 不存在"}、\texttt{"面 \{:?\} 不存在"}；
  \item 几何/语义类：\texttt{"挤出区域为空"}、\texttt{"仅支持三角面"}、
        \texttt{"面 \{:?\} 边界环长度=\{\}, 非三角面"}。
\end{itemize}
\textbf{半边字段路径保留原文}：\texttt{h.next} / \texttt{h.prev} / \texttt{twin.next} /
\texttt{twin.prev} / \texttt{p1.twin} / \texttt{n1.twin} / \texttt{n2.twin} /
\texttt{p2.twin} 等以字段名直接引用结构体成员，便于从错误信息定位到对应源码字段。
...
不创建 $F_{new2}$。同时维护边界环的 \texttt{next/prev} 链：记 $\mathrm{twin}$ 的前后
邻居为 $p, n$，则重链为
\[
  p \to (B \to M) \to (M \to A) \to n,
\]
并把 $p.\mathrm{next}$、$n.\mathrm{prev}$ 改指向新边界半边，否则边界环断裂、破坏拓扑。
...
\begin{itemize}
  ...
  \item $C \neq D$（防退化）；
  \item $C$ 与 $D$ 不相连（否则翻转会产生重复边/非流形，返回
        \texttt{FlipCreatesNonManifoldEdge}）。
\end{itemize}
...
\paragraph{边界顶点限制}
即使链接条件满足，$A$、$B$ 同时为边界顶点时折叠仍被拒绝。合并两个边界顶点
会让同一顶点在边界环中出现两次，破坏边界拓扑（产生非流形边界顶点）。

\subsection{操作步骤}

\begin{enumerate}
  \item \textbf{校验}：$h.\mathrm{face}, \mathrm{twin}.\mathrm{face}$ 均 $\mathrm{Some}$；
        $C \neq D$；链接条件满足；$A$、$B$ 不同时为边界顶点。
...
\begin{enumerate}
  ...
  \item \textbf{twin 配对}：首先一次性遍历所有现存半边，构建哈希索引
        $\text{boundary\_twin\_map}: (\text{origin}, \text{tip}) \to \text{HalfEdgeId}$，
        仅收录 twin 为边界半边（\texttt{face = None}）的内部半边。
        然后对每条新边 $h_i: \text{src} \to \text{dst}$，以 key $= (\text{dst}, \text{src})$
        在哈希中 $O(1)$ 查找满足条件的 $E$（$E$ 从 $\text{dst} \to \text{src}$，其 twin 从 $\text{src} \to \text{dst}$ 且为边界半边）。
        \begin{itemize}
...
\begin{tikzpicture}[
  ...
  \node[proc, below=of s, fill=blue!8] (p0) {一次性遍历全部半边，构建 \texttt{boundary\_twin\_map}：$(\text{origin}, \text{tip}) \to E$，仅含 twin 为边界者};
  \node[proc, below=of p0, fill=blue!4] (p1) {哈希查找 key $= (\text{dst}, \text{src})$，$O(1)$};
...
\begin{tabular}{@{}ll@{}}
  ...
  \texttt{add\_triangle}   & $O(|E_{\mathrm{he}}|)$（一次性构建哈希索引）$+ O(1)$（3 次查找）$+ O(|E|+|F|)$（校验） \\
...
\begin{longtable}{@{}ll@{}}
  ...
  \texttt{split\_boundary\_edge\_maintains\_boundary\_loop} & 真实 builder 边界边分裂后，边界环 next/prev 完整 \\
  \midrule
  ...
  \texttt{flip\_edge\_rejects\_when\_opposite\_vertices\_connected} & 四面体中 $(c,d)$ 已相连，翻转被拒绝 \\
  \midrule
  ...
  \texttt{collapse\_with\_both\_boundary\_vertices\_fails} & 对角线两端点均为边界顶点，拒绝折叠 \\
  \texttt{collapse\_interior\_edge\_with\_one\_boundary\_vertex} & 折叠「一个边界 + 一个内部」，边界环保持完整 \\
...
\begin{itemize}
  ...
  \item \textbf{为何 extrude\_region 边界顶点的 outgoing 指向 up？} 边界顶点 $v$
        ...
        因为它不依赖于具体的边界边排列顺序。
  \item \textbf{为何 add\_triangle 使用临时哈希而非持久化邻接表？}
        \texttt{add\_triangle} 每次调用时一次性遍历全部半边构建
        \texttt{boundary\_twin\_map}（$O(|E_{\mathrm{he}}|)$），然后 3 次查找 $O(1)$。
        相比旧版每条边独立扫描（$O(3|E_{\mathrm{he}}|)$）减少约 3 倍常数。
        持久化邻接表会增加 \texttt{MeshStorage} 内存开销并破坏与 \texttt{slotmap}
        的简洁集成；大规模重建仍应使用 \texttt{build\_mesh\_from\_polygons}（一次成型）。
