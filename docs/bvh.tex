...
\begin{tabular}{@{}lll@{}}
  ...
  \multirow{4}{*}{查询}
    ...
    & \texttt{bvh.faces\_in\_aabb(\&AABB, \&mesh)} & 返回 AABB 重叠的候选面（布尔运算用） \\
    & \texttt{bvh.ray\_count\_hits(o, d, \&mesh)}  & 射线命中三角面总数（射线奇偶法用） \\
  \midrule
  \multirow{3}{*}{统计}
...
\paragraph{为布尔运算新增的查询}
\texttt{faces\_in\_aabb} 与 \texttt{ray\_count\_hits} 专为
\texttt{boolean.rs} 的 corefinement 加速而新增：
\begin{itemize}
  \item \texttt{faces\_in\_aabb}：递归遍历 BVH，节点 AABB 与查询 AABB
        不重叠则剪枝；叶子节点内逐面计算 AABB 精筛。返回的只是
        \emph{候选集}，调用方仍需 \texttt{segment\_triangle\_intersection}
        做精确判定。复杂度 $O(\log F + k)$，$k$ 为候选面数。
  \item \texttt{ray\_count\_hits}：与 \texttt{ray\_intersection} 不同的
        是\textbf{统计所有命中}而非返回最近交点，因为射线奇偶法需要
        交点总数的奇偶性。复杂度 $O(\log F + k)$。
\end{itemize}
