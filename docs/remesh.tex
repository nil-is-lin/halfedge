...
\begin{tabular}{@{}lll@{}}
  ...
  \midrule
  核心   & \texttt{isotropic\_remesh(mesh, target, iter,} & 4 步迭代流水线 \\
         & \texttt{\quad reproject, preserve\_boundary)}  &               \\
  快捷   & \texttt{quick\_remesh(mesh)}                    & 自动目标长度 + 3 次迭代 \\
  快捷   & \texttt{remesh\_to\_length(mesh, L)}            & 指定目标长度 + 5 次迭代 \\
  统计   & \texttt{RemeshStats}                            & 迭代/分裂/折叠/翻转次数与目标长度 \\
  \bottomrule
\end{tabular}
\end{center}
...
涉及非流形端点的边\textbf{跳过}（见「非流形防护」）。边界边\textbf{跳过}：分裂边界边
会在边界环上插入中点、增加边界顶点，改变边界拓扑（见「边界保持」）。
...
涉及边界顶点的边\textbf{跳过}（见「边界保持」）。

\paragraph{非流形防护}
每轮迭代开始先扫描网格得到非流形顶点集合（判定依据：outgoing 环绕长度 $\neq$
真实入射半边数）。\texttt{split} / \texttt{collapse} / \texttt{flip} 三个阶段
对涉及非流形端点的边\textbf{统一跳过}，避免 \texttt{VertexRing} 只遍历一个扇区
导致链接条件误判、残留悬空引用、破坏拓扑。\texttt{collapse} 阶段还会在每次折叠
后重新校验合并出的新顶点，若其变为非流形则同步加入跳过集合。

\paragraph{边界保持}
参数 \texttt{preserve\_boundary}（默认 \texttt{true}）控制是否保持开放网格的边界不变：
\texttt{split} 跳过边界边（\texttt{is\_boundary\_edge}），避免在边界环上插入中点；
\texttt{collapse} 跳过任一端点为边界顶点的边（\texttt{edge\_touches\_boundary}），
既覆盖边界边，也覆盖「一个边界 + 一个内部」的内部边——折叠后者会移动/合并边界顶点、
破坏边界环。\texttt{flip} 本就要求两侧都有面（边界边不参与），\texttt{smooth} 本就
跳过边界顶点，故无需额外处理。设为 \texttt{false} 时允许 split/collapse 修改边界
（拓扑操作本身会维护边界环的 \texttt{next/prev} 链，见 \texttt{split\_edge} 文档）。

\paragraph{表面投影（reproject）}
当 \texttt{reproject = true} 时，迭代前快照原始表面为三角形列表；每轮平滑后，
对每个顶点求其到原始表面的最近点（点到三角面最近点，暴力 $O(VF)$）并投影回该点，
避免切向平滑导致体积收缩/形状漂移。
...
\paragraph{翻转合法性：对角线已相连检测}
翻转会把对角线 $(a,b)$ 换成 $(c,d)$。若 $c$、$d$ 已经相连，翻转后同一条边
会被 $4$ 个面共享，产生非流形边。因此\textbf{翻转前}先检测 $c$、$d$ 之间是否
已存在边（复杂度 $O(\deg)$），已相连则跳过该翻转。
...
\begin{tabular}{@{}ll@{}}
  ...
  \texttt{remesh\_preserves\_boundary\_vertices\_on\_open\_grid} & 开放网格 split/collapse 均不改动边界顶点集合 \\
  \texttt{remesh\_preserve\_boundary\_false\_allows\_boundary\_split} & \texttt{preserve\_boundary=false} 允许分裂边界边 \\
  \texttt{remesh\_reproject\_keeps\_vertices\_on\_unit\_sphere} & \texttt{reproject=true} 顶点投影回单位球面（半径 $\approx 1$）\\
  ...
  \texttt{nonmanifold\_vertices\_detects\_pinch} & 共享顶点的两个四面体：识别 pinch 顶点为非流形 \\
  \texttt{remesh\_collapse\_skips\_nonmanifold\_endpoints} & 折叠跳过非流形端点后，不引入非流形边/悬空引用 \\
  \texttt{remesh\_split\_flip\_skip\_nonmanifold\_vertices} & split/flip 同样跳过非流形顶点，重网格化后拓扑保持 \\
  \texttt{are\_vertices\_connected\_basic} & 两顶点间是否已存在边的判定正确 \\
  \bottomrule
\end{tabular}%
}
\end{center}

全模块共 \textbf{17 个}单元测试，全库共 \textbf{552 个}。
...
\begin{itemize}
  ...
  \item \textbf{为何 split/collapse/flip 都跳过非流形顶点？} \texttt{VertexRing}
        只能遍历非流形顶点的其中一个扇区，会令链接条件误判并残留悬空引用；
        三段统一跳过涉及非流形端点的操作，即可保证重网格化不破坏拓扑。collapse
        还额外对合并出的新顶点做二次校验，覆盖折叠过程中新产生的非流形顶点。
\end{itemize}
