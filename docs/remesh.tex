...
\paragraph{非流形端点防护}
折叠前先扫描网格得到非流形顶点集合（判定依据：outgoing 环绕长度 $\neq$
真实入射半边数）。若边的任一端点属于该集合（例如两个扇区共享一个顶点），
则\textbf{跳过该边}，避免 \texttt{VertexRing} 只遍历一个扇区导致链接条件
误判、残留悬空引用、破坏拓扑。
...
全模块共 \textbf{15 个}单元测试，全库共 \textbf{552 个}。
...
\begin{itemize}
  ...
  \item \textbf{为何折叠前跳过非流形顶点？} \texttt{VertexRing} 只能遍历
        非流形顶点的其中一个扇区，会令链接条件误判并残留悬空引用；跳过涉及
        非流形端点的边即可保证折叠不破坏拓扑。
\end{itemize}
