...
\begin{tabular}{@{}lll@{}}
  ...
  \multirow{4}{*}{通用遍历原语}
    ...
    & \texttt{collect\_outgoing\_halfedges\_into(v, buf)} & 同上，填充调用方 \texttt{buf}（\textbf{内存复用}） \\
    & \texttt{collect\_face\_halfedges\_into(f, buf)}     & 同上，填充调用方 \texttt{buf}（\textbf{内存复用}） \\
  \bottomrule
\end{tabular}
\end{center}
...
\textbf{通用遍历原语}（\texttt{pub fn}）：模块还导出两对底层收集函数，
...
\subsection{内存复用变体：\texttt{*\_into} 函数}
\label{sec:into-variants}

两个原语各有\textbf{内存复用}变体（后缀 \texttt{\_into}），签名如下：
\begin{itemize}
  \item \texttt{collect\_outgoing\_halfedges\_into(\&mesh, v, buf: \&mut Vec<HalfEdgeId>)}
  \item \texttt{collect\_face\_halfedges\_into(\&mesh, f, buf: \&mut Vec<HalfEdgeId>)}
\end{itemize}

\textbf{语义}：\texttt{buf.clear()} 后填充，\textbf{不释放已分配容量}。
返回 \texttt{Vec} 的版本内部即声明空 \texttt{buf}、调用 \texttt{\_into} 变体填充、
最后返回 \texttt{buf}，二者产出完全一致。

\textbf{适用场景}：热循环中反复遍历不同顶点 / 面的邻域，例如：
\begin{lstlisting}
let mut buf = Vec::new();
...
\textbf{与 lazy 的对比}：两者都降低分配开销，但侧重点不同——
\texttt{*\_into} 适合需要随机访问或迭代期 \texttt{\&mut mesh} 的场景（保留 eager 的解耦特性），
lazy 适合纯只读遍历（零堆分配，但持有借用）。

\begin{center}
\renewcommand{\arraystretch}{1.18}
\begin{tabular}{@{}lll@{}}
  \toprule
  \textbf{维度} & \texttt{*\_into}（内存复用） & \texttt{Lazy}（延迟） \\
  \midrule
  堆分配        & 首次分配，后续复用容量     & 零堆分配 \\
  迭代期借用    & 无（已释放）               & 持有 \texttt{\&MeshStorage} \\
  迭代期 \texttt{\&mut mesh} & \checkmark & $\times$ \\
  随机访问      & \checkmark（\texttt{buf[i]}） & $\times$ \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item \textbf{内存}：lazy 零堆分配；eager 在 1M 级别分配 $\sim$105 MB
        （顶点环 45 MB + 面边界 60 MB）。若需保留 eager 的「迭代期可 \texttt{\&mut mesh}」
        特性又想避免重复分配，可使用 \texttt{*\_into} 内存复用变体（见 \S\ref{sec:into-variants}）：
        在热循环外声明一次 \texttt{buf}，循环内 \texttt{buf.clear()} 后填充，容量自动复用。
...
\begin{longtable}{@{}ll@{}}
  ...
  \midrule
  \multicolumn{2}{l}{\textit{场景八：内存复用变体（*\_into）}} \\
  \texttt{collect\_outgoing\_halfedges\_into\_matches\_original} & \texttt{\_into} 产出与原函数一致（icosphere 全顶点） \\
  \texttt{collect\_face\_halfedges\_into\_matches\_original}     & \texttt{\_into} 产出与原函数一致（icosphere 全面） \\
  \texttt{collect\_outgoing\_halfedges\_into\_reuses\_capacity}  & \texttt{buf} 容量跨调用不收缩 \\
  \texttt{collect\_face\_halfedges\_into\_clears\_buffer}        & 预填垃圾数据后被 \texttt{clear}，长度 $=3$ \\
  \texttt{collect\_outgoing\_halfedges\_into\_isolated\_vertex}  & 孤立顶点 \texttt{buf} 保持空 \\
  \bottomrule
\end{longtable}
}

\texttt{cargo test} 全库共 \textbf{503 个}用例，全部通过（0 失败 / 0 警告）。
...
\begin{itemize}
  ...
  \item \textbf{何时使用 \texttt{*\_into} 内存复用变体？} 当满足\textbf{全部}以下条件时优选：
        (1) 热循环中反复遍历不同顶点 / 面；(2) 需要随机访问或迭代期 \texttt{\&mut mesh}
        （故不能用 lazy）；(3) 单次遍历结果用完即弃。此时在循环外声明一次 \texttt{buf}，
        循环内调用 \texttt{collect\_...\_into(\&mesh, id, \&mut buf)}，\texttt{buf} 容量自动复用，
        避免每轮重新分配。若仅需纯只读遍历，\texttt{lazy} 仍是更优选择（零堆分配）。
