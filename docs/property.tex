...
pub struct PropertyStore<T> {
    data: HashMap<usize, T>,  // key = slotmap key 的完整 64 位 ffi (idx + version)
}
\end{lstlisting}

\textbf{键的选取}：使用 slotmap key 的\textbf{完整 64 位 \texttt{as\_ffi()} 值}
（idx + version）作为 \texttt{usize} 键，而非仅低 32 位 idx。原因：

\begin{itemize}
  \item 同一元素的 idx + version 组合在其生命周期内唯一不变；
  \item slot 被复用时 version 递增，新元素得到与旧元素不同的键，
        旧属性条目\textbf{自动成为孤儿}（永远不会被新元素匹配到），
        避免了「slot 复用导致新元素静默继承旧属性」的隐患；
  \item 仍便于 \texttt{ErasedProperty::remove\_by\_index} 在不知类型时批量删除。
\end{itemize}

\textbf{孤儿条目}：删除元素后未调用 \texttt{remove\_*\_all\_props} 时，
旧属性条目不会消失，但因 key 中的 version 已过时，新元素永远不会命中它，
故\textbf{无正确性问题}，仅占用少量内存。如需立即释放，可调用
\texttt{PropertyStore::clear()} 或 \texttt{remove\_*\_all\_props}（见 \S\ref{sec:remove-callback}）。

\textbf{平台假设}：依赖 \texttt{usize} 为 64 位（64-bit 平台）。32-bit 平台上
\texttt{as\_ffi() as usize} 会截断高 32 位 version，回退到旧行为（需手动清理）。
本库目标平台为 64-bit，不做特殊处理。
...
再调 \texttt{as\_ffi()} 取\textbf{完整 64 位编码}（idx + version）作为键：
...
\textbf{对比旧设计}（仅取低 32 位 idx）：
\begin{center}
\renewcommand{\arraystretch}{1.20}
\begin{tabular}{@{}lll@{}}
  \toprule
  \textbf{} & \textbf{旧设计（仅 idx）} & \textbf{新设计（idx + version）} \\
  \midrule
  键值          & \texttt{ffi \& 0xFFFF\_FFFF} & \texttt{ffi}（完整 64 位） \\
  slot 复用行为 & 新元素继承旧属性（危险）       & 新元素得到新键，旧条目成孤儿（安全） \\
  删除后清理    & \textbf{必须}立即调用         & 可延迟（仅内存占用问题） \\
  平台依赖      & 无                            & 需 64-bit \texttt{usize} \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  \item \textbf{为何用 \texttt{Box<dyn ErasedProperty>} 而非 \texttt{Box<dyn Any>}？} \\
        ...
        显式调用 \texttt{remove}。
  \item \textbf{为何属性键用完整 \texttt{as\_ffi()} 而非仅 idx？} \\
        早期版本仅取低 32 位 idx 作键，slot 复用时新元素会\textbf{静默继承}
        旧元素的属性数据（\texttt{MeshStorage} 与 \texttt{MeshProperties} 解耦，
        用户调用 \texttt{mesh.remove\_vertex()} 不会触发属性清理），
        这是潜在的语义陷阱。改用完整 64 位 \texttt{as\_ffi()}
        （idx + version）作键后，slot 复用时 version 递增，
        新元素永远命中不到旧条目，旧条目成为孤儿（仅占少量内存，
        无正确性问题）。代价是依赖 64-bit \texttt{usize}，
        但本库目标平台已满足。
