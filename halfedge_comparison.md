...
| 库 | 运行时依赖数 | 核心依赖 | 编译复杂度 |
|---|---|---|---|
| **halfedge** | **5** | `slotmap`, `sprs`, `rayon`, `robust`, `log` | 低（`cargo build`，全为纯 Rust 依赖） |
...
**分析：** 本项目有 **5 个必需的运行时依赖**，均为纯 Rust、维护良好、无系统库要求的轻量 crate：`slotmap`（稳定句柄表）、`sprs`（稀疏矩阵，供 conformal / parameterization 使用）、`rayon`（并行迭代）、`robust`（Shewchuk 鲁棒谓词）、`log`（日志门面，零开销）。这并非"依赖最少"——OpenMesh（0）、libigl（2+）、tri-mesh（2+）都更少——但在"功能完整"这一档（CGAL / libigl / geometry-central / plexus）里属于**最精简之列**，且没有任何 C/C++ 系统库或 CMake / 工具链负担，编译速度极快、交叉编译友好、几乎不会遇到依赖冲突。CGAL 的依赖链是出了名的复杂（Boost + GMP + MPFR 在 Windows 上尤其痛苦），libigl 虽有 header-only 模式但完整功能依赖大量子模块。
...
2. **轻量纯 Rust 依赖：** 5 个必需依赖（`slotmap` / `sprs` / `rayon` / `robust` / `log`）均为纯 Rust、无系统库负担，编译速度极快、交叉编译友好
